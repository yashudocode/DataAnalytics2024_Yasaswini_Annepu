...
data("Smarket")
head(Smarket)
# data set consists ofpercentage returns for the S&P 500 stock index over 1, 250 days, from the #beginning of 2001 until the end of 2005
names(Smarket)
# For each date, we have recorded the percentage returns for each of the five previous
# trading days, "Lag1" through "Lag5".
# We have also recorded "Volume" (the number of shares traded on the previous day, in billions of dollars. # "Today" is the percentage return on the date
# "Direction" (whether the market was Up or Down on this date)
dim(Smarket)
summary(Smarket)
cor(Smarket)
# we observe Error in cor(Smarket) : 'x' must be numeric
# Convert Direction to numeric binary format
Smarket$Direction_numeric <- ifelse(Smarket$Direction == "Up", 1, 0)
...
# correlation between today’s returns and previous days’ returns. 
...
# We can see that the smallest p-value here is associated with Lag1. 
...
# can be used to produce a confusion matrix in order to determine how many # observations were correctly or incorrectly classified.
table(glm.pred, Direction)

# The diagonal elements of the confusion matrix indicate correct predictions,
# while the off-diagonals represent incorrect predictions
# Hence our model correctly predicted that the market would go up on 507 days and that # it would go down on 145 days, for a total of 507 + 145 = 652 correct predictions (507+145)/1250 = 0.5216
...
# Next, in order to implement the strategy i.e.,we can fit the model using part of the data,
#and then examine how well it predicts the held out data. 
#we will first create a vector corresponding to the observations from 2001 through 2004. 
...
#in our data set. The elements of the vector that correspond to observations 
#that occurred before 2005 are set to TRUE, whereas those that correspond 
#to observations in 2005 are set to FALSE. The object train is a Boolean vector, 
#since its elements are TRUE and FALSE.
#Boolean vectors can be used to obtain a subset of the rows or columns of a 
#matrix. For instance, the command Smarket[train,] would pick out a submatrix 
#of the stock market data set, corresponding only to the dates before 2005, 
#since those are the ones for which the elements of train are TRUE.

#The ! Symbol can be used to reverse all of the elements of a Boolean vector.
#That is, !train is a vector similar to train, except that the elements that 
#are TRUE in train get swapped to FALSE in !train, and the elements that are 
#FALSE in train get swapped to TRUE in !train. Therefore, Smarket[!train,] 
#yields a submatrix of the stock market data containing only the observations 
#for which train is FALSE—that is, the observations with dates in 2005. 
...
# Finally, we compute the predictions for 2005 and compare them to the actual 
...
#just Lag1 and Lag2, which seemed to have the highest predictive power in the 
#original logistic regression model.

glm.fit.model3 <- glm(Direction ~ Lag1 + Lag2, data = Smarket, 
                      family = binomial, subset = train)
